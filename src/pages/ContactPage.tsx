    ...
    <div className="py-20 min-h-screen surface-gradient">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="text-center mb-24">
          ...
          <h1 className="text-5xl font-serif text-primary md:text-7xl mb-8 font-bold leading-tight">
        ...
        <div className="grid lg:grid-cols-2 gap-20">
          <motion.div
            ...
            className="p-12 bg-card border-2 border-primary/20 rounded-[2.5rem] shadow-none relative h-fit group hover:border-primary hover:shadow-strong transition-all duration-300"
          >
            ...
            {!submitted ? (
              <form className="space-y-8" onSubmit={handleSubmit}>
                <div className="grid sm:grid-cols-2 gap-8">
                  <div>
                    <label className="block text-[10px] font-black text-primary mb-3 font-mono uppercase tracking-[0.2em]">{t("contact.form.name")}</label>
                    <input required name="name" type="text" placeholder={t("contact.form.placeholder_name")} className="w-full px-6 py-5 rounded-xl bg-transparent border-2 border-border focus:border-primary focus:ring-4 focus:ring-primary/5 outline-none transition-all font-medium text-lg placeholder:text-muted-foreground/50" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-black text-primary mb-3 font-mono uppercase tracking-[0.2em]">{t("contact.form.email")}</label>
                ...
                <div>
                  <label className="block text-[10px] font-black text-primary mb-3 font-mono uppercase tracking-[0.2em]">{t("contact.form.subject")}</label>
                  <input required name="subject" type="text" placeholder={t("contact.form.placeholder_subject")} className="w-full px-6 py-5 rounded-xl bg-transparent border-2 border-border focus:border-primary focus:ring-4 focus:ring-primary/5 outline-none transition-all font-medium text-lg placeholder:text-muted-foreground/50" />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-primary mb-3 font-mono uppercase tracking-[0.2em]">{t("contact.form.message")}</label>
                  <textarea required name="message" rows={5} placeholder={t("contact.form.placeholder_message")} className="w-full px-6 py-5 rounded-xl bg-transparent border-2 border-border focus:border-primary focus:ring-4 focus:ring-primary/5 outline-none transition-all resize-none font-medium text-lg placeholder:text-muted-foreground/50" />
                </div>
                {error && (
                ...
                <button disabled={isSubmitting} type="submit" className="w-full py-6 bg-primary text-primary-foreground font-black text-[12px] tracking-[0.3em] uppercase rounded-2xl flex items-center justify-center gap-4 hover:bg-accent hover:text-accent-foreground transition-all shadow-xl group disabled:cursor-not-allowed disabled:opacity-70 cursor-pointer">
                  ...
                  <Send className="w-6 h-6 group-hover:translate-x-2 group-hover:-translate-y-2 transition-all duration-300" />
                </button>
              </form>
            ) : (
              <motion.div
              ...
              >
                ...
                <h3 className="text-3xl font-serif font-bold text-primary mb-4">{t("contact.success.title")}</h3>
                <p className="text-muted-foreground italic">{t("contact.success.desc")}</p>
                <button
                  onClick={() => setSubmitted(false)}
                  className="mt-10 text-[10px] font-black uppercase tracking-widest text-accent border-b-2 border-accent pb-1 cursor-pointer"
                >
          ...
          <div className="flex flex-col gap-10">
            <motion.div whileHover={{ y: -6, scale: 1.008 }} transition={{ type: "spring", stiffness: 400, damping: 25 }} className="p-12 bg-primary text-primary-foreground rounded-[2.5rem] shadow-strong relative overflow-hidden group hover:shadow-[0_24px_50px_-12px_rgba(0,0,0,0.35)] transition-all duration-300">
              ...
              <div className="relative z-10">
                <h3 className="text-3xl font-serif mb-12 font-bold decoration-accent decoration-2">{t("contact.direct")}</h3>
                <div className="space-y-12">
                  <div className="flex items-center gap-8">
                    ...
                    <div>
                      <p className="text-[10px] font-mono tracking-[0.3em] uppercase font-black text-accent mb-2">{t("contact.speak")}</p>
                  ...
                  <div className="flex items-center gap-8">
                    ...
                    <div>
                      <p className="text-[10px] font-mono tracking-[0.3em] uppercase font-black text-accent mb-2">{t("contact.digital")}</p>
            ...
            <motion.div whileHover={{ y: -6, scale: 1.008 }} transition={{ type: "spring", stiffness: 400, damping: 25 }} className="p-12 bg-card border-2 border-primary/20 rounded-[2.5rem] shadow-none group hover:border-accent/40 hover:shadow-strong relative overflow-hidden transition-all duration-300 cursor-pointer">
              <div className="flex items-start gap-8">
                ...
                <div>
                  <p className="text-[10px] font-mono tracking-[0.3em] uppercase font-black text-primary mb-2">{t("contact.hub")}</p>
                  <p className="text-2xl font-serif text-primary leading-tight font-bold whitespace-pre-line">{t("contact.address")}</p>
                </div>
              </div>
            </motion.div>
          </div>
